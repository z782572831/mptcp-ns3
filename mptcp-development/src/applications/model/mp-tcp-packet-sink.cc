...
MpTcpPacketSink::StartApplication()    // Called at time specified by Start
{
  ...
  outfile.open("draw/all.txt",ios::out);
  outfile << "";
  outfile.close();
...
void 
MpTcpPacketSink::CalculateThroughput(){
  double totalRx = GetTotalRx();
  double temp = totalRx - preDataAmount;
  preDataAmount = totalRx;
  std::ofstream outfile;
  outfile.open("draw/all.txt",ios::app);
  outfile << Simulator::Now() <<" "<< totalRx << " " << temp << " " << (double)temp * 8 / 0.0005 <<std::endl;
  outfile.close();
  Simulator::Schedule(Seconds(0.0005),&MpTcpPacketSink::CalculateThroughput,this);
}
