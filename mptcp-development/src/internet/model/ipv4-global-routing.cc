  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      if (m_randomEcmpRouting)
      ...
      else if (m_flowEcmpRouting && allRoutes.size() > 1)
        {
          ...
          if(selectIndex==0) totalRx0+=ipPayload->GetSize();
          else totalRx1+=ipPayload->GetSize();
          Ptr<Node> node = m_ipv4->GetObject<Node>();
          uint32_t node_id = node->GetId();
          if(!flag && node_id == 2){
            CalculateThroughput();
...
void
Ipv4GlobalRouting::CalculateThroughput()
{
  std::ofstream outfile;
  outfile.open("draw/flow0.txt",ios::out);
  outfile<<"";
  outfile.close();
  outfile.open("draw/flow1.txt",ios::out);
  outfile<<"";
  outfile.close();
  Simulator::Schedule(Seconds(0.000),&Ipv4GlobalRouting::CalculateThroughput0,this);
  Simulator::Schedule(Seconds(0.000),&Ipv4GlobalRouting::CalculateThroughput1,this);
  
}
void
Ipv4GlobalRouting::CalculateThroughput0()
{
  double temp = totalRx0 - preRx0;
  preRx0 = totalRx0;
  std::ofstream outfile;
  outfile.open("draw/flow0.txt",ios::app);
  outfile << Simulator::Now() <<" "<< totalRx0 << " " << temp << " " << (double)temp * 8 / 0.0005 <<std::endl;
  outfile.close();
  Simulator::Schedule(Seconds(0.0005),&Ipv4GlobalRouting::CalculateThroughput0,this);
  
}
void
Ipv4GlobalRouting::CalculateThroughput1()
{

  double temp = totalRx1 - preRx1;
  preRx1 = totalRx1;
  std::ofstream outfile;
  outfile.open("draw/flow1.txt",ios::app);
  outfile << Simulator::Now() <<" "<< totalRx1 << " " << temp << " " << (double)temp * 8 / 0.0005 <<std::endl;
  outfile.close();
  Simulator::Schedule(Seconds(0.0005),&Ipv4GlobalRouting::CalculateThroughput1,this);
}
